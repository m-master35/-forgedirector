...
Amazon Nova 2 Lite samples short videos at roughly one frame per second. AWS documentation estimates about 2,880 input tokens for a 10-second video and 8,640 input tokens for a 30-second video. AWS has published Nova 2 Lite rates around $0.30/M input tokens and $2.50/M output tokens. Our live 3-second CI analyses have used roughly 3.5K–4K total tokens, so inference cost is comfortably below the proposed per-analysis selling price even after increasing launch quotas.

Use the actual Bedrock `usage` object returned in ForgeDirector responses to measure production cost and latency on real 10–60 second customer videos before increasing quotas again.
